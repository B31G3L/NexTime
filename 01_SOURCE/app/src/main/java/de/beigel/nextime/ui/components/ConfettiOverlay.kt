...
// Konfetti-Farben passend zum NexTime-Stil
...
fun ConfettiOverlay(
    ...
    LaunchedEffect(active) {
        if (active) {
            particles = List(120) { createParticle() }
        }
    }
    ...
    LaunchedEffect(active) {
        ...
        while (true) {
            ...
            particles = particles
                .map { p ->
                    p.copy(
                        x         = p.x + p.velocityX,
                        y         = p.y + p.velocityY,
                        velocityY = p.velocityY + 0.15f, // Schwerkraft
            ...
            // Neue Partikel nach Burst noch kurz nachschießen
